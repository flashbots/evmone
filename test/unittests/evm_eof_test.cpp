...
TEST_P(evm, eof_jumpi_inside_code_section)
{
    rev = EVMC_SHANGHAI;
    auto code = eof_bytecode(jumpi(14, 1) + OP_INVALID + OP_JUMPDEST + mstore8(0, 1) + ret(0, 1));
...
TEST_P(evm, eof_jumpi_into_header)
{
    rev = EVMC_SHANGHAI;
    // data section is of size 0x5b = OP_JUMPDEST
    const auto code = eof_bytecode(jumpi(9, 1), 0x5b * bytecode{0});
...
TEST_P(evm, eof_jumpi_into_data_section)
{
    rev = EVMC_SHANGHAI;
    // data section contains OP_JUMPDEST + mstore8(0, 1) + ret(0, 1)
    const auto code = eof_bytecode(jumpi(17, 1), OP_JUMPDEST + mstore8(0, 1) + ret(0, 1));
